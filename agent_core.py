...
GENERAL_SYSTEM_PROMPT = """你是一个通用中文 AI 助手，回答要准确、清楚、实用。

要求：
- 默认用中文回答，除非用户要求其他语言。
- 不要编造你不知道的实时信息。
- 涉及代码、学习、写作、解释、计划时，直接给可执行答案。
- 回答篇幅和问题复杂度匹配，不要为了显得正式而绕圈。
"""
...
def get_required_config(name: str, aliases: tuple[str, ...] = ()) -> str:
    ...
    raise ValueError(f"缺少环境变量: {all_names}")
...
def wants_attraction(user_prompt: str) -> bool:
    keywords = ("景点", "旅游", "旅行", "游玩", "去哪", "哪里玩", "路线", "攻略")
    return any(keyword in user_prompt for keyword in keywords)


def wants_weather_or_travel(user_prompt: str) -> bool:
    weather_keywords = (
        "天气", "气温", "温度", "下雨", "降雨", "雨", "晴", "多云", "阴天", "预报",
        "冷不冷", "热不热", "穿什么", "适合出门", "适合去",
    )
    return wants_attraction(user_prompt) or any(keyword in user_prompt for keyword in weather_keywords)
...
def build_llm_unavailable_answer(error_text: str) -> str:
    if "缺少环境变量" in error_text:
        return f"普通 AI 问答暂不可用：{error_text}。请先配置可访问的大模型 API。"
    return "普通 AI 问答暂不可用：当前后端无法连通大模型 API。天气、旅行等工具查询仍可继续使用。"


def answer_general_query(user_prompt: str, progress_cb=None) -> dict[str, str | bool | int]:
    if progress_cb:
        progress_cb(1, 2, "读取模型配置")

    try:
        llm = create_llm_client()
        if progress_cb:
            progress_cb(2, 2, "生成回答")
        answer = llm.generate(user_prompt, system_prompt=GENERAL_SYSTEM_PROMPT, verbose=False)
        if answer.startswith("错误:调用语言模型服务时出错"):
            answer = build_llm_unavailable_answer(answer)
    except Exception as exc:
        answer = build_llm_unavailable_answer(str(exc))
...
def answer_query(user_prompt: str, progress_cb=None) -> dict[str, str | bool | int]:
    if not wants_weather_or_travel(user_prompt):
        return answer_general_query(user_prompt, progress_cb=progress_cb)

    include_attraction = wants_attraction(user_prompt)
    total_steps = 4 if include_attraction else 3

    if progress_cb:
        progress_cb(1, total_steps, "识别城市并查询天气")
...
请给用户一个简洁中文答复。"""
    ...
    return {
        ...
        "task_type": "天气 + 旅行" if include_attraction else "天气",
    }
